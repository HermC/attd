    ...
    var app = new Vue({
        ...
        methods:{     
        	...
        	getData:function(){
				...
				var env = this;
				return $.get('{{ route('api_outdoors') }}',param ).done(function(result){
...
@section('content')
        ...
        <div class="weui-panel__hd">外勤记录   <span  style="float:right;margin: -22px -10px -15px 0px;"><a href="{{ route('outdoor') }}"  class="weui-btn weui-btn_mini weui-btn_primary" >提交外勤</a>&nbsp;&nbsp;<a href="javascript:;" class="weui-btn weui-btn_mini weui-btn_primary  open-popup"  data-target="#search" >搜索</a></span></div>
        	...
        	<div class="pull-wrap">
				...
				@verbatim
		          <div class="weui-media-box weui-media-box_text"  v-for="att in attendants">
		            <h4 class="weui-media-box__title">{{ att.vftime }}  {{ att.location }}</h4>
		            <p class="weui-media-box__desc"  >备注:{{ att.memo }} </p>
		            <ul class="weui-media-box__info">
		              <li class="weui-media-box__info__meta">{{ att.vtime }}</li>
		            </ul>
		          </div>
		          @endverbatim
